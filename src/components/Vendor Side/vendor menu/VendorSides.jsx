...
const VendorSides = () => {
    ...
    // Custom foods
    useEffect(() => {
        axios.get(`https://api-chow.onrender.com/api/vendors/menu/custom/sides/${vendorId}`, { headers })
            .then(response => {
                console.log(response)
    ...
    const handleDelete = (_id) => {
        console.log(_id)
        axios.delete(`https://api-chow.onrender.com/api/vendors/menu/sides/${_id}`, { headers })
            .then(res => {
                console.log(res)
                Swal.fire({
                    icon: 'success',
